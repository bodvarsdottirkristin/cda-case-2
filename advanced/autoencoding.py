...
print(f"Windows: {X_raw.shape}  —  {X_raw.shape[0]} windows × {WINDOW_SIZE}s × {len(signal_cols)} signals")
...
# =========================
# 4. Conv1D Autoencoder
# =========================
LATENT_DIM = 32

def _conv_out_size(window: int) -> int:
    """Compute flattened encoder size after 3 stride-2 convolutions."""
    t = window
    t = (t + 2*2 - 5) // 2 + 1   # conv1: k=5, s=2, p=2
    t = (t + 2*2 - 5) // 2 + 1   # conv2: k=5, s=2, p=2
    t = (t + 2*1 - 3) // 2 + 1   # conv3: k=3, s=2, p=1
    return 128 * t, t


class Conv1DAutoencoder(nn.Module):
    def __init__(self, n_channels: int, latent_dim: int, window: int):
        super().__init__()
        flat_size, t_out = _conv_out_size(window)
        self.t_out = t_out
    ...
    def forward(self, x):
        ...
        # trim to input length in case ConvTranspose1d adds extra steps
...
model     = Conv1DAutoencoder(n_channels=C, latent_dim=LATENT_DIM, window=WINDOW_SIZE).to(DEVICE)
...
plt.tight_layout()
out = FIGURES_DIR / 'autoencoder_tsne.png'
...
plt.tight_layout()
out2 = FIGURES_DIR / 'autoencoder_tsne_cohort.png'
