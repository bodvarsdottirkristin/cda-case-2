"""
Cluster questionnaire variables only using fixed k = 3.

Purpose:
    Sanity check whether self-reported questionnaire answers themselves
    separate the three experimental phases:

        phase1 = rest
        phase2 = puzzle/stress
        phase3 = recovery

If questionnaire-only clusters with k=3 do not align with Phase, then expecting
...
Output:
    advanced/outputs/questionnaire_clustering_k3/
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Cluster questionnaire variables only using fixed k=3."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ADVANCED_DIR / "outputs" / "questionnaire_clustering_k3",
        help="Output folder.",
    )
...
def main() -> None:
    ...
    metrics = {
        ...
        "note": (
            "Clusters were fitted using questionnaire variables only with fixed k=3. "
            "This tests whether self-reported emotions themselves separate the three "
            "experimental phases."
        ),
    }
